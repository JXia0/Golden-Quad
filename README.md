...
当前七场景流程及新增操作见 [成长过渡与研究室引导](Docs/RestoredInterludesAndResearchGuide.md)，核心选择后果见 [可改变后续的玩法版本](Docs/MeaningfulChoicesPlayable.md)。

最新操作与交接见 [舞台表演与可重做的研究实验](Docs/PerformanceAndLearning.md)。

- A / D、方向键：行走；Space / W / ↑：跳跃。
- 按住 E：牵手、携带、呼吸；松开 E：放手、放下、发声。
- 玩具：按住 E 上弦，用 A / D 朝向，松开滚过去；孩子会先滚回来，接住后再滚给他。
- 舞台：按住 E 时 A / D 控制发声朝向，选择长短两拍及回应哪侧听众；可换位置演唱，Q 改写，F 请观众接唱；结束时 F 主动谢幕。
- F：操作机关、窗户、线索和门；带着玩具离开幼儿园会改变研究室可用的工具。
- 研究：探索上层档案、复用箱子或板开门搭路，以灯、玩具或牵手帮助模型；空手 Q 召回，它会记住被帮助走过的路，可撤掉帮助重试。带报告回工作台才提交结果。
- 结尾：研究室中陪伴或放手的实际做法决定对方等待你还是主动离开，门前仍可重新牵手。
- R：回到当前检查点；结尾 Enter：重玩。
- 成长过渡一：F 按铃下车，A/D 走到学校；E 举手后松开，F 继续。
- 成长过渡二：A/D 选纸片，按住 E 后移动、松开放回轮廓；拼好后放到左边寄出或右边留存，F 继续。
- 研究室：目标常驻；H 查看或收起额外提示。
